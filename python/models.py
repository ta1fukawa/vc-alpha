...
class EmbedModel1d(torch.nn.Module):
    def __init__(self, model_type, n_freq, n_frames):
        ...
        self.conv1b = torch.nn.Conv1d(512, 512, kernel_size=3, dilation=2, padding='same')
        self.drop1  = torch.nn.Dropout(p=0.2)

        self.conv2a = torch.nn.Conv1d(512, 512, kernel_size=3, dilation=3, padding='same')
        self.conv2b = torch.nn.Conv1d(512, 512, kernel_size=3, dilation=3, padding='same')
        self.drop2  = torch.nn.Dropout(p=0.2)

        self.conv3a = torch.nn.Conv1d(512, 512, kernel_size=3, dilation=1, padding='same')
        self.conv3b = torch.nn.Conv1d(512, 512, kernel_size=3, dilation=1, padding='same')
        self.drop3  = torch.nn.Dropout(p=0.2)

        # self.conv4a = torch.nn.Conv1d(512, 512, kernel_size=3, dilation=1, padding='same')
        # self.conv4b = torch.nn.Conv1d(512, 512, kernel_size=3, dilation=1, padding='same')
...
class EmbedModel2d(torch.nn.Module):
    def __init__(self, model_type, n_freq, n_frames, pool_size=(2, 4)):
        ...
        self.conv1a = torch.nn.Conv2d(1, 16, kernel_size=(5, 5), dilation=(1, 1), padding='same')
        self.conv1b = torch.nn.Conv2d(16, 16, kernel_size=(5, 5), dilation=(1, 1), padding='same')
        self.drop1  = torch.nn.Dropout2d(p=0.2)
        self.pool1  = torch.nn.MaxPool2d(kernel_size=pool_size)
        
        self.conv2a = torch.nn.Conv2d(16, 64, kernel_size=(5, 5), dilation=(1, 1), padding='same')
        self.conv2b = torch.nn.Conv2d(64, 64, kernel_size=(5, 5), dilation=(1, 1), padding='same')
        self.drop2  = torch.nn.Dropout2d(p=0.2)
        self.pool2  = torch.nn.MaxPool2d(kernel_size=pool_size)
        
        self.conv3a = torch.nn.Conv2d(64, 256, kernel_size=(5, 5), dilation=(1, 1), padding='same')
        ...
        self.conv4a = torch.nn.Conv2d(256, 1024, kernel_size=(5, 5), dilation=(1, 1), padding='same')
        self.conv4b = torch.nn.Conv2d(1024, 1024, kernel_size=(5, 5), dilation=(1, 1), padding='same')
        self.drop4  = torch.nn.Dropout2d(p=0.2)
        self.pool4  = torch.nn.MaxPool2d(kernel_size=pool_size)
        
        self.conv5a = torch.nn.Conv2d(1024, 2048, kernel_size=(5, 5), dilation=(1, 1), padding='same')
        self.conv5b = torch.nn.Conv2d(2048, 2048, kernel_size=(5, 5), dilation=(1, 1), padding='same')
        self.drop5  = torch.nn.Dropout2d(p=0.2)
        self.pool5  = torch.nn.MaxPool2d(kernel_size=pool_size)
        
        # self.conv6a = torch.nn.Conv2d(2048, 8192, kernel_size=(5, 5), dilation=(1, 1), padding='same')
        # self.conv6b = torch.nn.Conv2d(8192, 8192, kernel_size=(5, 5), dilation=(1, 1), padding='same')
        # self.drop6  = torch.nn.Dropout2d(p=0.2)
        # self.pool6  = torch.nn.MaxPool2d(kernel_size=pool_size)
        
        if self.model_type == 'stats_pooling':
            self.conv7  = torch.nn.Conv2d(2048, 2048, kernel_size=(5, 5), dilation=(1, 1), padding='same')
            self.line7  = torch.nn.Linear(4096, 512)
        elif self.model_type == 'linear':
            self.conv7  = torch.nn.Conv2d(2048, 2048, kernel_size=(5, 5), dilation=(1, 1), padding='same')
            self.line7  = torch.nn.Linear(2048 * n_freq * n_frames // np.prod(pool_size)**5, 512)
